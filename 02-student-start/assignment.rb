...
class Solution
  ...
  def load_collection(file_path)
    #place solution here
    file = self.class.load_hash(file_path)
    @coll.insert_many(file)
  end
  ...
  def find_by_name(fname, lname)
    #place solution here
    @coll.find(first_name: fname, last_name: lname).projection(_id: 0 ,first_name: 1, last_name: 1, number: 1)
  end
  ...
  def find_group_results(group, offset, limit)
    #place solution here
    @coll.find(group: group).projection(_id: 0 , group: 0).sort(secs: 1).skip(offset).limit(limit)
  end
